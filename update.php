update nomtable set valeur where
...
if (isset($_POST['send'])) {
      if (
            ...
            $insert = $bd->prepare("UPDATE vendre set idl=?,idap=?,datedeb=?, datefin=?, comment=? WHERE datedeb=$id");
            $insert->execute(
                  array(
                        $_POST['type'],
                        $_POST['app'],
...
<body class="">
      ...
      <main class="form-signin d-flex justify-content-center mt-4">
            <form class="container w-50 pt-3 border mt-5 rounded" method="POST" action="">
                  ...
                  <div class="mt-2 mb-3">
                        <select name="type" class="form-select">
                              ...
                              <?php while ($louer = $locataire->fetch()) { ?>
                                    <option value="<?= $louer['idl'] ?>"><?= $louer['noml'] ?><?= $louer['prenoml'] ?></option>
                  ...
                  <div class="mt-2 mb-3">
                        <textarea type="text" name="commentaire" class="form-control" id="" cols="35" rows="1" placeholder="Commentaire (facultatif)"></textarea>
                  </div>
